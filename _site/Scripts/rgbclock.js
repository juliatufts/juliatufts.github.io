	...
	Clock.prototype = {
		draw: function(screen, size, time) {
			...
			function updateTime(){
				...
				seconds = currentDate.getSeconds() + (currentDate.getMilliseconds() / 1000);
			}
			...
			function drawMinuteArc(screen, handLength){
				...
				var minutesOnlyStart = Math.min(minutes, seconds);
				drawArc(screen, minutesOnlyStart, minutes, handLength, "rgba(0, 255, 0, 1)");

				var minutesSecondsEnd = Math.min(minutes, seconds);
				drawArc(screen, 0, minutesSecondsEnd, handLength, "rgba(0, 255, 255, 1)");
			}
			...
			function drawHourArc(screen, handLength){
				...
				var hoursOnly = Math.min(hours, Math.max(seconds, minutes));
				drawArc(screen, hoursOnly, hours, handLength, "rgba(255, 0, 0, 1)");

				var hoursSecondsStart = Math.min(hours, minutes);
				var hoursSecondsEnd = Math.max(hoursSecondsStart, Math.min(hours, seconds));
				drawArc(screen, hoursSecondsStart, hoursSecondsEnd, handLength, "rgba(255, 0, 255, 1)");

				var hoursMinutesStart = Math.min(hours, seconds);
				var hoursMinutesEnd = Math.max(hoursMinutesStart, Math.min(hours, minutes));
				drawArc(screen, hoursMinutesStart, hoursMinutesEnd, handLength, "rgba(255, 255, 0, 1)");

				var everythingEnd = Math.min(hours, seconds, minutes);
				drawArc(screen, 0, everythingEnd, handLength, "rgba(255, 255, 255, 1)");	
